...
def eval_epoch(model, test_dataloader, device, args=None):
	...
	# #################################################################
	print(f'test data : {len(test_dataloader)}\n')
	...
	with torch.no_grad():
		...
		# print(f'visual shape {(batch_visual_output_list[0].shape)} sequence shape{(batch_sequence_output_list[0].shape)}\n\n')
		...
		# diff = []
		# for i in range(1000):
		# 	tmp = np.argmax(sim_matrix.T[i])
		# 	if i != tmp:
		# 		print(i + 2, tmp + 2)
		# 		diff.append(i + 2)
		# print(f'text idx : {diff}\n')
...
def _run_on_single_gpu(model, batch_list_t, batch_list_v, batch_sequence_output_list, batch_visual_output_list, args=None):
	...
	for idx1, b1 in enumerate(batch_list_t):
		...
		for idx2, b2 in enumerate(batch_list_v):
			...
			#print(f'sequence_output : {sequence_output.shape}, visual_output : {visual_output.shape}\n')
			b1b2_logits, *_tmp = model.get_similarity_logits(sequence_output, visual_output, input_mask, video_mask)
			#print(f'b1b2 shape : {b1b2_logits.shape}\n')
