...
from .sampling import multi_segments_sampling, uniform_sampling, vss_sampling, low_k_sampling
...
class RawVideoExtractorpyAV():
	def __init__(self, centercrop=False, size=224, is_train=True, num_segments=12,
		...
		self.target_video = [23, 32, 73, 79, 153, 166, 220, 235, 281, 286, 393, 412, 413, 420, 435, 437, 462, 482, 506, 567, 593, 655, 660, 687, 725, 740, 830, 880, 935, 958]
		self.data = pd.read_csv('/home/estela19/workspace/vlret/dataset/msrvtt/msrvtt_data/MSRVTT_JSFUSION_test.csv')
	...
	def get_video_data(self, video_path, video_id, start_time=None, end_time=None, random_shift=None):
		...
		if self.train:
			...
			# uni_inds = uniform_sampling(self.num_segments, num_frames, twice_sample=False)
			inds, bound = vss_sampling(self.num_segments, num_frames, video_id)
			# inds = low_k_sampling(self.num_segments, num_frames, video_id)
		...
		# for idx in self.target_video:
		# 	vid = self.data['video_id'].values[idx-2]
		# 	if video_id == vid:
		# 		print(f'vid {vid}')
		# 		save_path = f'/home/estela19/workspace/vlret/result/{vid}'
		# 		if not os.path.exists(save_path):
		# 			os.makedirs(save_path)

		# 		bound = bound[1:-1]
		# 		for i in inds:
		# 			all_frames[i].to_image().save(f'{save_path}/vss_{i}.jpg')
		# 		for i in bound:
		# 			i = i[0]
		# 			all_frames[i].to_image().save(f'{save_path}/bound_{i}.jpg')
		# 		for i in uni_inds:
		# 			all_frames[i].to_image().save(f'{save_path}/uni_{i}.jpg')
