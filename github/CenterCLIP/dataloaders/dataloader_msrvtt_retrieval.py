	...
	def _get_rawvideo(self, choice_video_ids):
		...
		for i, video_id in enumerate(choice_video_ids):
			...
			raw_video_data, slice_len = self.rawVideoExtractor.get_video_data(
										os.path.join(self.features_path, "{}.mp4".format(video_id)), video_id
										)
...
if __name__=='__main__':
	...
	for i, j in zip(video, misret):
		res['video_id'] = data['video_id'].values[i]
		res['ours'] = data['sentence'].values[i]
		res['centerclip'] = data['sentence'].values[j]
